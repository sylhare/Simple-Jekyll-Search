(function(global, factory) {
	...
	function createStateChangeListener(xhr, callback) {
		return function() {
			if (xhr.readyState === 4 && xhr.status === 200) try {
	...
	var OptionsValidator = class {
		constructor(params) {
			if (!this.validateParams(params)) throw new Error("-- OptionsValidator: required options missing");
		...
		validate(parameters) {
			...
			this.requiredOptions.forEach((requiredOptionName) => {
				if (typeof parameters[requiredOptionName] === "undefined") errors.push(requiredOptionName);
			});
			return errors;
		}
	...
	function findLiteralMatches(text, criteria) {
		...
		if (pattern.filter((word) => lowerText.indexOf(word) >= 0).length !== pattern.length) return [];
		const matches = [];
	...
	function findFuzzyMatches(text, criteria) {
		...
		if (criteriaIndex !== criteria.length) return [];
	...
	function buildWildcardFragment(config) {
		...
		if (maxSpaces === 0) return "[^ ]*";
		if (maxSpaces === Infinity) return "[^ ]*(?: [^ ]*)*";
		return `[^ ]*(?: [^ ]*){0,${maxSpaces}}`;
	}
	...
	var SearchStrategy = class {
		constructor(findMatchesFunction) {
			this.findMatchesFunction = findMatchesFunction;
		}
		matches(text, criteria) {
			if (text === null || text.trim() === "" || !criteria) return false;
			return this.findMatchesFunction(text, criteria).length > 0;
		}
		findMatches(text, criteria) {
			if (text === null || text.trim() === "" || !criteria) return [];
			return this.findMatchesFunction(text, criteria);
		}
	};
	var LiteralSearchStrategy = new SearchStrategy(findLiteralMatches);
	var FuzzySearchStrategy = new SearchStrategy((text, criteria) => {
	...
	var WildcardSearchStrategy = class extends SearchStrategy {
		constructor(config = {}) {
	...
	var HybridSearchStrategy = class extends SearchStrategy {
		constructor(config = {}) {
			super((text, criteria) => {
		...
		applyFuzzyConstraints(matches, criteria) {
			...
			if (normalizedCriteriaLength === 0) return matches;
			return matches.filter((match) => {
				const normalizedMatchLength = this.normalizeLength(match.text);
				return Math.max(0, normalizedMatchLength - normalizedCriteriaLength) <= limit;
			});
		}
	...
	var DefaultHybridSearchStrategy = new HybridSearchStrategy();
	var StrategyFactory = class {
		static create(config = { type: "literal" }) {
			const { options } = config;
			switch (this.isValidStrategy(config.type) ? config.type : "literal") {
				case "literal": return LiteralSearchStrategy;
				case "fuzzy": return FuzzySearchStrategy;
				case "wildcard": return new WildcardSearchStrategy(options);
				case "hybrid": return new HybridSearchStrategy(options);
				default: return LiteralSearchStrategy;
			}
		}
	...
	var Repository = class {
		...
		put(input) {
			if (isObject(input)) return this.addObject(input);
			if (Array.isArray(input)) return this.addArray(input);
		}
		clear() {
			this.data.length = 0;
			return this.data;
		...
		search(criteria) {
			if (!criteria) return [];
			return this.findMatches(this.data, criteria).sort(this.options.sortMiddleware).map((item) => ({ ...item }));
		}
		...
		addArray(arr) {
			const added = [];
			this.clear();
			for (const item of arr) if (isObject(item)) added.push(this.addObject(item)[0]);
			return added;
		}
		...
		findMatchesInObject(obj, criteria) {
			...
			const result = { ...obj };
			result._matchInfo = {};
			for (const key in obj) if (!this.isExcluded(obj[key]) && this.options.searchStrategy.matches(obj[key], criteria)) {
				hasMatch = true;
				if (this.options.searchStrategy.findMatches) {
					const matchInfo = this.options.searchStrategy.findMatches(obj[key], criteria);
					if (matchInfo && matchInfo.length > 0) result._matchInfo[key] = matchInfo;
				}
			}
		...
		searchStrategy(strategy) {
			if (!strategy?.type || !StrategyFactory.isValidStrategy(strategy.type)) return LiteralSearchStrategy;
			return StrategyFactory.create(strategy);
		}
		normalizeStrategyOption(strategy) {
			if (!strategy) return this.getDefaultStrategyConfig();
		...
		getDefaultStrategyConfig() {
			...
			if (typeof defaultStrategy === "string") return { type: defaultStrategy };
	...
	function setOptions(_options) {
		if (_options.pattern) options.pattern = _options.pattern;
		if (_options.template) options.template = _options.template;
	...
	var SimpleJekyllSearch$1 = class {
		constructor() {
			...
			this.optionsValidator = new OptionsValidator({ required: REQUIRED_OPTIONS });
		}
		debounce(func, delayMillis) {
			if (delayMillis) {
				if (this.debounceTimerHandle) clearTimeout(this.debounceTimerHandle);
		...
		initWithURL(url) {
			load(url, (err, json) => {
				if (err) this.throwError(`Failed to load JSON from ${url}: ${err.message}`);
		...
		getStoredSearchState() {
			try {
				...
				if (Date.now() - state.timestamp > 1800 * 1e3) {
					this.clearSearchState();
					return null;
				}
		...
		restoreSearchState() {
			if (this.options.resultsContainer.children.length > 0) return;
			let query = this.options.searchInput.value?.trim();
			if (!query) query = this.getStoredSearchState() || "";
			if (query.length > 0) {
		...
		init(_options) {
			if (this.optionsValidator.validate(_options).length > 0) this.throwError(`Missing required options: ${REQUIRED_OPTIONS.join(", ")}`);
			...
			if (isJSON(this.options.json)) this.initWithJSON(this.options.json);
			else this.initWithURL(this.options.json);
			const rv = {
	...
	function highlightWithMatchInfo(text, matchInfo, options = {}) {
		if (!text || matchInfo.length === 0) return escapeHtml(text);
		...
		if (maxLength && result.length > maxLength) result = truncateAroundMatches(text, mergedMatches, maxLength, options.contextLength || 30, className);
		return result;
	}
	function truncateAroundMatches(text, matches, maxLength, contextLength, className) {
		if (matches.length === 0) return escapeHtml(text.substring(0, maxLength - 3)) + "...";
	...
	var DEFAULT_TRUNCATE_FIELDS = [
		"content",
		"desc",
		"description"
	];
	var DEFAULT_NO_HIGHLIGHT_FIELDS = [
		"url",
		"link",
		"href",
		"query"
	];
	function createHighlightTemplateMiddleware(options = {}) {
		...
		return function(prop, value, _template, query, matchInfo) {
			...
			if (noHighlightFields.includes(prop)) return;
			const shouldTruncate = truncateFields.includes(prop);
			if (matchInfo && matchInfo.length > 0 && query) {
				const highlighted = highlightWithMatchInfo(value, matchInfo, {
					...highlightOptions,
					maxLength: shouldTruncate ? highlightOptions.maxLength : void 0
				});
				return highlighted !== value ? highlighted : void 0;
			}
			if (shouldTruncate && highlightOptions.maxLength && value.length > highlightOptions.maxLength) return escapeHtml(value.substring(0, highlightOptions.maxLength - 3) + "...");
		};
	}
	function defaultHighlightMiddleware(prop, value, template, query, matchInfo) {
		return createHighlightTemplateMiddleware()(prop, value, template, query, matchInfo);
	}
	function SimpleJekyllSearch(options) {
		return new SimpleJekyllSearch$1().init(options);
	}
	...
	exports.DefaultHybridSearchStrategy = DefaultHybridSearchStrategy;
	exports.HybridSearchStrategy = HybridSearchStrategy;
	exports.StrategyFactory = StrategyFactory;
	exports.createHighlightTemplateMiddleware = createHighlightTemplateMiddleware;
	exports.default = SimpleJekyllSearch;
	exports.defaultHighlightMiddleware = defaultHighlightMiddleware;
	exports.escapeHtml = escapeHtml;
	exports.highlightWithMatchInfo = highlightWithMatchInfo;
	exports.mergeOverlappingMatches = mergeOverlappingMatches;
});
